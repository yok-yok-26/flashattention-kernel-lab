...
// Define your __global__ kernel(s), device helpers, and launch_flashattention_v2
...
void warp_all_reduce_max_sum(unsigned mask, float& max_val, float& sum_val){
  ...
  sum_val *= __expf(max_val_old - max_val);
...
__global__ void flashattention_v2(FlashAttentionParams params) {
  ...
  auto tileq_cur = [&](int row_idx, int col_idx) -> float& {
    return smem_q[row_idx * D + col_idx];
  };
  ...
  // load q tile      16 * 64 / 128 = 8
  float* q_tile = const_cast<float*>(params.q + offset + blockIdx.x * BLOCK_M * D);
  for (int iloop_m = 0; iloop_m < BLOCK_M; iloop_m+=1)
  {
    tileq_cur(iloop_m, tx) = *(q_tile + iloop_m * D + tx);
  }
  ...
  for (int iloop_n = 0; iloop_n < N; iloop_n += BLOCK_N){
    ...
    {
        *reinterpret_cast<float4*>(&smem_k[tx / 2][(tx % 2) * 8 + 0]) = 
        *reinterpret_cast<float4*>(k_tile + iloop_d + (tx / 2) * D + (tx % 2) * 8 + 0);
        *reinterpret_cast<float4*>(&smem_k[tx / 2][(tx % 2) * 8 + 4]) = 
        *reinterpret_cast<float4*>(k_tile + iloop_d + (tx / 2) * D + (tx % 2) * 8 + 4);
      ...
      {
        float a0 = tileq_cur(idx_tile_m * 8 + 0, iloop_d + i);
        float a1 = tileq_cur(idx_tile_m * 8 + 1, iloop_d + i);
        float a2 = tileq_cur(idx_tile_m * 8 + 2, iloop_d + i);
        float a3 = tileq_cur(idx_tile_m * 8 + 3, iloop_d + i);
        float a4 = tileq_cur(idx_tile_m * 8 + 4, iloop_d + i);
        float a5 = tileq_cur(idx_tile_m * 8 + 5, iloop_d + i);
        float a6 = tileq_cur(idx_tile_m * 8 + 6, iloop_d + i);
        float a7 = tileq_cur(idx_tile_m * 8 + 7, iloop_d + i);
    ...
    for (int irow = 0; irow < 8; irow++){
      ...
      for (int icol = 0; icol < 4; icol++){
        sum_val += __expf(tmp[irow][icol] * params.scale - max_val);
      }
      ...
      if ((tx & 15) == 0)
      {
        ...
        if (smem_max_old[idx_tile_m * 8 + irow] < smem_max[idx_tile_m * 8 + irow])
        {
          smem_sum[idx_tile_m * 8 + irow] = 
            smem_sum[idx_tile_m * 8 + irow] * 
            __expf(smem_max_old[idx_tile_m * 8 + irow] - smem_max[idx_tile_m * 8 + irow]) + sum_val;
        }
        else
        {
          smem_sum[idx_tile_m * 8 + irow] += 
            sum_val * __expf(smem_max[idx_tile_m * 8 + irow] - smem_max_old[idx_tile_m * 8 + irow]);
        }
      }
      __syncthreads();

      for (int icol = 0; icol < 4; icol++){
        tmp[irow][icol] = __expf((tmp[irow][icol] * params.scale - smem_max[idx_tile_m * 8 + irow]));
      }
    ...
    for (int iloop_d = 0; iloop_d < D; iloop_d += BLOCK_D)
    {
      ...
      *reinterpret_cast<float4*>(v_tile + iloop_d + (tx / 2) * D + (tx % 2) * 8 + 4);
      ...
      {
        o_acc[id][0][0] = tmp2[0][0] + __expf(
          smem_max_old[idx_tile2_m * 4 + 0] - smem_max[idx_tile2_m * 4 + 0]
        ) * o_acc[id][0][0];
        o_acc[id][0][1] = tmp2[0][1] + __expf(
          smem_max_old[idx_tile2_m * 4 + 0] - smem_max[idx_tile2_m * 4 + 0]
        ) * o_acc[id][0][1];
      }
      else
      ...
      {
        o_acc[id][1][0] = tmp2[1][0] + __expf(
          smem_max_old[idx_tile2_m * 4 + 1] - smem_max[idx_tile2_m * 4 + 1]
        ) * o_acc[id][1][0];
        o_acc[id][1][1] = tmp2[1][1] + __expf(
          smem_max_old[idx_tile2_m * 4 + 1] - smem_max[idx_tile2_m * 4 + 1]
        ) * o_acc[id][1][1];
      }
      else
      ...
      {
        o_acc[id][2][0] = tmp2[2][0] + __expf(
          smem_max_old[idx_tile2_m * 4 + 2] - smem_max[idx_tile2_m * 4 + 2]
        ) * o_acc[id][2][0];
        o_acc[id][2][1] = tmp2[2][1] + __expf(
          smem_max_old[idx_tile2_m * 4 + 2] - smem_max[idx_tile2_m * 4 + 2]
        ) * o_acc[id][2][1];
      }
      else
      ...
      {
        o_acc[id][3][0] = tmp2[3][0] + __expf(
          smem_max_old[idx_tile2_m * 4 + 3] - smem_max[idx_tile2_m * 4 + 3]
        ) * o_acc[id][3][0];
        o_acc[id][3][1] = tmp2[3][1] + __expf(
          smem_max_old[idx_tile2_m * 4 + 3] - smem_max[idx_tile2_m * 4 + 3]
        ) * o_acc[id][3][1];
      }
      else
...
cudaError_t launch_flashattention_v2(const FlashAttentionParams& params,
  ...
  dim3 block{128};
  size_t dsbytes = 64 * 128 * sizeof(float);
  CUDA_CHECK(cudaFuncSetAttribute(
      flashattention_v2<>, cudaFuncAttributeMaxDynamicSharedMemorySize,
      static_cast<int>(dsbytes)));
  flashattention_v2<><<<grid, block, dsbytes, stream>>>(params);
  CUDA_KERNEL_CHECK();
