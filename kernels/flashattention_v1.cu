...
// Define your __global__ kernel(s), device helpers, and launch_flashattention_v1
...
__global__ void flashattention_v1(FlashAttentionParams params) {
...
cudaError_t launch_flashattention_v1(const FlashAttentionParams& params,
  ...
  dim3 block{128};
  flashattention_v1<<<grid, block, 0, stream>>>(params);
  CUDA_KERNEL_CHECK();
